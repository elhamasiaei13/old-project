...
public class DateJsonSerializer extends JsonSerializer<Object> {
    ...
    public void serialize(Object date, JsonGenerator json, SerializerProvider provider)
		...
		if(true){
			Locale locale = LocaleUtil.fromLanguageId("fa");// LocaleContextHolder.getLocale();
			
			String localValue = StringPool.BLANK;
			
			if(date instanceof LocalDate){
				localValue = DateUtil.getDate((LocalDate) date, locale, "gregorian", ZoneIdUtil.getDefault());

				
			}
			else if(date instanceof LocalDateTime){
				localValue = DateUtil.getDateTime((LocalDateTime) date, locale, "gregorian", ZoneIdUtil.getDefault());
