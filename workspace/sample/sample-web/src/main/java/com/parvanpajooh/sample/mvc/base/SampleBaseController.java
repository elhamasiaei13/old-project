package com.parvanpajooh.sample.mvc.base;
...
public abstract class SampleBaseController {
	...
	/*protected AmUserInfo getUserInfo(HttpServletRequest request) throws ObjectNotFoundException, ParvanServiceException {

		HttpSession session = request.getSession();

		AmUserInfo amUserInfo = (AmUserInfo) session.getAttribute("userInfo");
		...
		if (amUserInfo == null) {
			...
			if ("anonymousUser".equals(userName)) {
				...
				amUserInfo = AmUserInfo.getGuestUserInfo(ip, contextPath, tenantId, locale);

			} else {

				UserInfo userInfo = ((IdmUserDetails) authentication.getPrincipal()).getUserInfo();
				userInfo.setIp(ip);
				...
				employeeVO = employeeService.findByUsername(userInfo);

				if (employeeVO != null) {
					agentId = employeeVO.getGeneralAgent().getId();
					
					List<GeneralAgentVO> childrenAgent;
					childrenAgent = generalAgentService.findByParentId(userInfo, agentId);
					for (GeneralAgentVO agent : childrenAgent) {
						agentChildrenIds.add(agent.getId());
					}
				}

				amUserInfo = AmUserInfo.getDefaultUserInfo(
						userInfo.getUserId(), 
						userName, 
				...
				// TODO set additionalData to userInfo
			}

			session.setAttribute("userInfo", amUserInfo);
		}
	...
	/*private UserVO loadUserVOFullForUserInfoByUsername(String userName, String tenantId) throws ParvanServiceException{
		
		UserVO userVO = null;
		
		if(Config.getBooleanProperty("ejbCalls", true)){
			userVO = idmServiceFacade.loadUserFullForUserInfByUsername(userName, tenantId);
			
		} else {
			Map<String, Object> data = loadUserFullForUserInfByUsername(userName, tenantId);
			
			if(data != null){
				ObjectMapper mapper = mapperProvider.getDefaultMapper();
				userVO = mapper.convertValue(data, UserVO.class);
			}
