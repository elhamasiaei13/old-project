	...
	public void delete(ComplaintCategory complaintCategory) throws ParvanServiceException {
		...
		try {
			...
			if (Validator.equals(hasChildren, true)) {
				throw new ParvanDaoException(ErrorCode.COMPLAINTMANAGEMENT_CATEGORY_DELETE__CATEGORY_EXISTS_IN_ANOTHER_CATEGORY);
			}
			
			complaints = complaintRepository.findByComplaintCategoryId(complaintCategoryId);
			if(Validator.isNotNull(complaints)){
